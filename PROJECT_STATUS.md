...
**Statusas**: 🟢 **BLOG AUTOMATION blog-gen VEIKIA** (s28 — ESM crash + Edge timeout fix, Node+Fluid Compute, env vars iš Empirra; draft branch+Telegram E2E success) 🟡 publish grandinė BLOCKED (Telegram webhook→empirra.com) · 🟢 **Marinos klausimyno Q2/Q25/Q28/Q39 korekcijos LIVE** (s28, +email regex bug fix) · 🟢 9 LIVE blog straipsniai (s28 +duomenų subjekto teisės) · 🟢 **Hanken Grotesk visoje svetainėje** (s27) · 🟢 **BDAR audito klausimynas LIVE** (s24) · 🟢 21 indeksuojami seo/* (s25) · 🟢 Supabase+Resend+OpenAI Production · 🟡 scoring matrica laukia MasterLegal
...
| `POST /api/forms/bdar-audit` | 🟢 **LIVE Production** (s24, Node) | **BDAR audito klausimynas** — validate(honeypot+origin+rate-limit+consent) → scoreAnswers() → runPrompt() AI išvada (gpt-4.1) → Supabase `bdar_audit_responses` insert + newsletter upsert → Resend email klientui + Veriva. E2E patikrinta production. Rate limit 3/min per IP (KI-014: per-isolate). **s29: Q2 kontaktai išskaidyti į 3 atskirus laukus** (asmuo/`type=email`/`type=tel`) — email iš `kontaktinis-el-pastas` lauko (ne regex iš laisvo teksto, kuris lūždavo nuo skyrybos). Q2 E2E su naujais laukais NETESTUOTA. |
...
| `POST /api/automations/blog-gen` | 🟡 Kodas OK, **CRON SUSTABDYTAS** (s29 `41971ae`) | Cron blog post generation: topics.json → AI (OpenAI gpt-4.1) → validators → Pexels hero → template injection → GitHub draft branch → Telegram. **s28 atblokuotas**: Node + Fluid Compute (180s), ESM fix (tsconfig CommonJS), AI timeout 140s — `success:true` E2E. **s29: `crons` laukas PAŠALINTAS iš vercel.json** (publish blokuota webhook→empirra, auto-gen kūrė draft be naudos). Endpointas lieka — rankinis trigger galimas. |
| `POST /api/automations/telegram-webhook` | 🟡 PRODUCTION kodas OK, webhook → empirra.com | Telegram callback handler: P → blog-approve, R → Supabase state, S → delete branch. **s28: Node runtime, kodas veikia (GET→200), BET Telegram bot webhook nukreiptas į `empirra.com` (bendras bot'as) → Veriva callback'ai neateina.** Supabase `veriva_telegram_revise_state` nepatikrinta. |
| `POST /api/automations/blog-approve` | 🟡 PRODUCTION kodas OK, E2E netestuotas | Publish flow: addBlogCardToGrid + linkInternal + updateSitemap + status=published + mergeBranchToMain + Telegram. **s28: Node runtime, auth+GitHub token write izoliuotai patikrinti (201/204/404), BET realus telegram→approve→merge srautas NETESTUOTAS** (webhook→empirra blokeris). |
...
| GitHub API | 🟢 GITHUB_TOKEN LIVE (s28) — write veikia | Branch create/commit/merge per blog automation pipeline, repo `riko8825/Veriva-geras`. Fine-grained PAT, Contents+PR Read/write. Patikrinta: POST /git/refs 201, /merges 404 (auth OK), contents 200. **NE Sensitive** (Empirra Sensitive — nenuoseklumas). |
