...
Before polishing, gather Veriva project context:

1. **Read** `CLAUDE.md` (projekto autoritetas — stack, brand, taisyklės)
2. **Read** `index.html` `:root` design tokens (CSS variables — `--ink`, `--blue2`, `--gold2`, `--ffd`, `--ff`)
3. **Invoke** `emil-design-eng` skill if you need deeper design philosophy context
4. **Determine** quality bar: MVP (greitas pataisymas) vs flagship (pilnas polish pass)

Brand context (Veriva default):
- Tonas: Profesionalus, autoritetingas, faktais paremtas. Be marketing'inio bullshit'o.
- Šriftai: Syne (display, `--ffd`), Plus Jakarta Sans (body, `--ff`)
- Spalvos: `--ink #07111f`, `--blue2 #2255e0`, `--gold2 #e8b84b`, `--cream #f8f7f4`
- Stack: vanilla HTML/CSS/JS (NE React/Tailwind)
