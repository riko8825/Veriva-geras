...
| `POST /api/automations/blog-gen` | 🟢 PRODUCTION (s28) — VEIKIA E2E | Cron blog post generation: topics.json → AI (OpenAI gpt-4.1) → validators → Pexels hero → template injection → GitHub draft branch → Telegram. **s28 atblokuotas**: Node runtime + Fluid Compute (maxDuration 180s), ESM crash fix (tsconfig CommonJS), AI timeout 140s. Testas `success:true`: draft branch + uniqueness 89-93 + Telegram. Env vars (GITHUB_TOKEN write, PEXELS, TELEGRAM ×2) iš Empirra. |
